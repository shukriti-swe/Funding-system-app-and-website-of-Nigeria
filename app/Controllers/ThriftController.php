...
class ThriftController extends BaseController
{
    ...
    public function createThrift()
    {
        ...
        $get_setting = json_decode($get_referral_setting['settings_value'],true);
        
        $today_date = date("Y-m-d");
        $date=date_create($today_date);
        date_add($date,date_interval_create_from_date_string($get_setting['thrift_start_days']." days"));
        $thrift_start_date_less = date_format($date,"Y-m-d");

        $date=date_create($today_date);
        date_add($date,date_interval_create_from_date_string($get_setting['thrift_start_not_later']." days"));
        $thrift_start_date_grater = date_format($date,"Y-m-d");
        
        if($thrift_start_date_less <=$input['start_date']){
            $ins_data['thrift_group_start_date'] =  $input['start_date'];
           
        }else{
            $error = $this->generateError(lang('Thrift.short_start_date'));
            return $this
                ->getResponse(
                    $error,
                    ResponseInterface::HTTP_OK
        ...
        if($thrift_start_date_grater >=$input['start_date']){
            $ins_data['thrift_group_start_date'] =  $input['start_date'];
        }else{
            $error = $this->generateError(lang('Thrift.long_start_date'));
            return $this
                ->getResponse(
                    $error,
                    ResponseInterface::HTTP_OK
    ...
    public function joinThrift()
    {
        ...
        if ($thrift_group) {
            ...
            */
            ...
            $queryt = $tbuilder->get();
            $check_thrift_group = $queryt->getrow();
    
    
            if ($check_thrift_group) {
                $success = $this->generateSuccess(lang('Thrift.already_joined_thrift'), $thrift_group);
...
//            }
//
//            // Referral is not requried at this moment, So commented out
//            //$this->addReferralBonus($thrift_group);
//
//            $this->checkInvitation($thrift_group_id);

            $sucess = $this->generateSuccess(lang('Thrift.proceed_join'), $thrift_group);
    ...
    public function addReferralBonus($thrift_group){
        ...
        if(CURRENT_USER['referred_by']>0){
            $amount = ($thrift_group->thrift_group_product_price * 5)/100;
    ...
    public function listThrifts()
    {
        ...
        $builder->select('t2.*, t1.thrift_group_id, t1.thrift_group_member_join_date, t3.first_name, t3.last_name, t3.user_profile_image');
        ...
        $others->select('t2.*, t3.first_name, t3.last_name, t3.user_profile_image');
