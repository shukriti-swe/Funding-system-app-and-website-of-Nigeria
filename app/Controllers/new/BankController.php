...
class BankController extends BaseController
{
    ...
    public function webHookPaystack(){
        ...
        $get_user_bvn =   $user_bvn_model->where('user_id', 10)->first();

        // IF Success add that bank to user bank .. Added Temporarily
        if($update_data['status'] == 1){
            $data['user_id'] = $get_user_bvn['user_id'];
            $data['bank_code'] = $get_user_bvn['bank_code'];
            $data['account_number'] = $get_user_bvn['account_number'];
            $data['beneficiary'] = $get_user_bvn['beneficiary'];           
        }
        ///        
