...
class Thrift{
    ...
    public function getThriftWinner($thriftid){
        ...
        $nbuilder->select('tw.thrift_group_id as thrift_id, u.id, u.first_name, u.last_name, u.email, u.user_profile_image, u.user_gender');
    ...
    public function getUserActiveThriftAmount($userid){
        //Get all the thrifts user is a member of
        $nbuilder = $this->db->table('thrift_two_win tw');
        $nbuilder->join('thrift_two_win_member tm', 'tw.id = tm.thrift_group_id');
        $nbuilder->where('tm.thrift_group_member_id ', $userid);
        $nbuilder->Where('tw.thrift_group_open', 1);
        ...
        if($query){
            ...
            foreach ($query->getResult() as $row) {
                $amount+=$row->thrift_group_product_price;
            }
        }
